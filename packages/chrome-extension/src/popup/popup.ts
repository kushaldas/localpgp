/**
 * LocalPGP Chrome Extension - Popup Script
...
async function sendMessage(action: string, data?: Record<string, unknown>): Promise<{
  success: boolean;
  data?: unknown;
  error?: string;
}> {
...
async function saveAllowedOrigins(): Promise<void> {
  return new Promise((resolve) => {
    chrome.storage.local.set({ [ALLOWED_ORIGINS_KEY]: allowedOrigins }, () => {
      resolve();
    });
  });
}
